...
Audit timestamp: **2026-03-04** (`cargo fmt --check`, `cargo clippy --workspace --all-targets -- -D warnings`, and `cargo test --workspace` passed via `rch`).
...
| Primitive semantics (87 ops) | parity_green | `Primitive` enum (87 ops) in `crates/fj-core/src/lib.rs`; evaluator + extensive tests in `crates/fj-lax/src/lib.rs`; all workspace tests green | Continue expanding oracle-backed primitive fixture families |
...
| `grad` transform semantics | parity_green | Tape-based reverse-mode AD in `crates/fj-ad/src/lib.rs`; 68/77 primitive VJP+JVP rules (9 linalg/FFT stubs); custom_vjp/custom_jvp registration; Jacobian/Hessian; value_and_grad shared forward pass | Close remaining linalg/FFT VJP stubs |
...
| RaptorQ sidecar durability pipeline | in_progress | Durability implementation in `crates/fj-conformance/src/durability.rs`; CLI in `crates/fj-conformance/src/bin/fj_durability.rs`; sidecar/scrub/proof artifacts under `artifacts/durability/` | Expand automated sidecar generation to all long-lived evidence bundles |
...
| AD completeness | parity_green | 68/77 primitives with VJP+JVP rules; custom_vjp/custom_jvp registration; Jacobian/Hessian matrix computation; value_and_grad; ReduceWindow VJP fully implemented | Close remaining 9 linalg/FFT VJP stubs (Cholesky, Qr, Svd, TriangularSolve, Eigh, Fft, Ifft, Rfft, Irfft) |
...
| Special functions + linear algebra stubs | parity_gap | Cbrt, Lgamma, Digamma, ErfInv, IsFinite, IntegerPow, Nextafter implemented; Cholesky/Qr/Svd/TriangularSolve/Eigh/Fft/Ifft/Rfft/Irfft are eval stubs | Implement linalg/FFT evaluation kernels |
