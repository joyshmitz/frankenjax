...
- Rust bench command: `cargo bench -p fj-core --bench core_baseline` with the
  `core/tensor_stack_axis0` and `core/tensor_repeat_axis0` filters, local
  same-host execution.
- JAX oracle: `benchmarks/jax_comparison/.venv/bin/python`
- JAX/JAXLIB: 0.10.1 / 0.10.1, `jax_enable_x64=true`, CPU backend
...
- JAX domination score (same-host corrected/measured estimate): ~40/100 — stack
  axis0 (0.083x), repeat_axis0 (0.276x), slice (corrected ~0.72x), and
