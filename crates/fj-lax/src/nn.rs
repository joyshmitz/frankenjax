...
/// Log-sigmoid computed in the DIRECT grouping `-log(1 + exp(-x))` (the naive
/// `Neg → Exp → Add(1) → Log → Neg` graph), NOT the stable `-softplus(-x)` grouping of
/// [`log_sigmoid`] (which differs in the last bit). Uses only `exp`/`ln`, both bit-identical
/// to the `Exp`/`Log` primitives (as proven by the softmax/log-softmax superinstructions), so
/// this matches the decomposed graph bit-for-bit. Used by the interpreter log-sigmoid
/// superinstruction. Elementwise → threads flat.
#[must_use]
pub fn log_sigmoid_direct(x: &[f64]) -> Vec<f64> {
...
/// L2-normalize a single row in place, `x / sqrt(Σ x²)`, writing into `dst`. Bit-identical to the
/// decomposed `Mul(x,x) → ReduceSum → Sqrt → BroadcastInDim → Div` graph: an index-order sum of
/// `x[i]·x[i]`, then `f64::sqrt` (= the `Sqrt` primitive), then a per-element TRUE division
/// `x[i] / norm` (matching the `Div` primitive — NOT a reciprocal-multiply, which would differ in the
/// last bit). Callers guarantee equal-length `src`/`dst`.
fn l2_normalize_row_into(src: &[f64], dst: &mut [f64]) {
    let mut sum_sq = 0.0;
    for &value in src {
        sum_sq += value * value;
    }
...
/// L2-normalize along the last axis of a 2D array, `x / ‖x‖₂` per row (`jax.nn.normalize` with the
/// default L2 order, no epsilon). Output keeps the input shape [rows,cols] (rank-PRESERVING, unlike
/// the reducing `l2_norm_2d`). ROW-PARALLEL and BIT-IDENTICAL to the decomposed
/// `Mul(x,x) → ReduceSum(axis=1) → Sqrt → BroadcastInDim → Div` graph via [`l2_normalize_row_into`]:
/// index-order sum-of-squares, correctly-rounded `f64::sqrt`, then true per-element division. Rows
/// are independent so only the outer loop is threaded, above the `softmax_2d_thread_count` work gate.
/// Used by the interpreter L2-normalize superinstruction.
#[must_use]
pub fn l2_normalize_2d(x: &[f64], rows: usize, cols: usize) -> Vec<f64> {
    let len = checked_2d_row_major_len("2D l2_normalize", x, rows, cols);
    ...
    if threads <= 1 {
        for i in 0..rows {
            let start = i * cols;
            l2_normalize_row_into(&x[start..start + cols], &mut result[start..start + cols]);
        }
        return result;
    }
    ...
    std::thread::scope(|scope| {
        ...
        while row0 < rows {
            ...
            scope.spawn(move || {
                ...
                {
                    l2_normalize_row_into(src_row, dst_row);
                }
            });
        }
    });
...
/// Row-wise L1 (Manhattan / taxicab) norm `Σ |x|`, bit-identical to the decomposed
/// `Abs(x) → ReduceSum` graph: an index-order sum of `x[i].abs()` (`f64::abs` = the `Abs` primitive).
/// Callers guarantee a non-empty row.
fn l1_norm_row(x: &[f64]) -> f64 {
    let mut s = 0.0;
    for &v in x {
        s += v.abs();
    }
...
/// Row-wise L1 norm along the last axis of a 2D array, one scalar per row
/// (`jax.numpy.linalg.norm(x, ord=1, axis=-1)`). ROW-PARALLEL and BIT-IDENTICAL to the decomposed
/// `Abs(x) → ReduceSum(axis=1)` graph: the per-row sum-of-absolutes is an index-order accumulation via
/// [`l1_norm_row`] (matching `Abs` then the primitive `ReduceSum`). Only the outer row loop is threaded
/// (rows independent), above the `softmax_2d_thread_count` work gate, so per-row summation order is
/// preserved. Used by the interpreter L1-norm superinstruction. NOTE: 1-input `Σ|x|`, distinct from
/// the 2-input Manhattan distance `Σ|a-b|`.
#[must_use]
pub fn l1_norm_2d(x: &[f64], rows: usize, cols: usize) -> Vec<f64> {
    let _len = checked_2d_row_major_len("2D l1_norm", x, rows, cols);
    let mut result = vec![0.0; rows];
    if cols == 0 || rows == 0 {
        return result;
    }
    ...
    if threads <= 1 {
        for (i, slot) in result.iter_mut().enumerate() {
            let start = i * cols;
            *slot = l1_norm_row(&x[start..start + cols]);
        }
        return result;
    }
    ...
    std::thread::scope(|scope| {
        ...
        while row0 < rows {
            ...
            scope.spawn(move || {
                for (k, out_slot) in out_block.iter_mut().enumerate() {
                    let start = k * cols;
                    *out_slot = l1_norm_row(&x_block[start..start + cols]);
                }
            });
        }
    });
...
/// Mean squared error of a single pair of rows, `Σ(a - b)² / n`, bit-identical to the decomposed
/// `Sub -> Mul(square) -> ReduceSum -> Div(n)` graph.
fn mean_squared_error_row(a: &[f64], b: &[f64]) -> f64 {
    let mut sum_sq = 0.0;
    for (&av, &bv) in a.iter().zip(b.iter()) {
        let diff = av - bv;
        sum_sq += diff * diff;
...
pub fn mean_squared_error_2d(a: &[f64], b: &[f64], rows: usize, cols: usize) -> Vec<f64> {
    let _ = checked_2d_row_major_len("2D mean_squared_error a", a, rows, cols);
    let _ = checked_2d_row_major_len("2D mean_squared_error b", b, rows, cols);
    let mut result = vec![0.0; rows];
    if cols == 0 || rows == 0 {
        return result;
    }
    ...
    if threads <= 1 {
        for (i, slot) in result.iter_mut().enumerate() {
            let start = i * cols;
            *slot = mean_squared_error_row(&a[start..start + cols], &b[start..start + cols]);
        }
        return result;
    }
...
/// Mean absolute error along the last axis of two 2D arrays, one scalar per row. ROW-PARALLEL and
/// BIT-IDENTICAL to the decomposed `Sub(a,b) -> Abs -> ReduceSum(axis=1) -> Div(n)` graph via
/// [`mean_absolute_error_row`]: the absolute-error accumulator is an index-order sum matching
/// `ReduceSum`, and the trailing `/ n` matches the scalar `Div`. Used by the interpreter MAE
/// superinstruction.
#[must_use]
pub fn mean_absolute_error_2d(a: &[f64], b: &[f64], rows: usize, cols: usize) -> Vec<f64> {
    let _ = checked_2d_row_major_len("2D mean_absolute_error a", a, rows, cols);
    let _ = checked_2d_row_major_len("2D mean_absolute_error b", b, rows, cols);
    let mut result = vec![0.0; rows];
    if cols == 0 || rows == 0 {
        return result;
    }
    ...
    if threads <= 1 {
        for (i, slot) in result.iter_mut().enumerate() {
            let start = i * cols;
            *slot = mean_absolute_error_row(&a[start..start + cols], &b[start..start + cols]);
        }
        return result;
    }
        ...
        while row0 < rows {
            ...
            scope.spawn(move || {
                for (k, out_slot) in out_block.iter_mut().enumerate() {
                    let start = k * cols;
                    *out_slot =
                        rmse_row(&a_block[start..start + cols], &b_block[start..start + cols]);
                }
            });
        }
    });
...
fn cross_entropy_row(p: &[f64], q: &[f64]) -> f64 {
    let mut sum = 0.0;
    for (&pv, &qv) in p.iter().zip(q.iter()) {
        sum += pv * qv.ln();
    }
...
/// Cross-entropy from probabilities `-Σ p·log(q)` along the last axis of two 2D arrays, one scalar per
/// row (`p`=target probabilities, `q`=predicted probabilities — the soft-label / knowledge-distillation
/// classification loss `H(p,q)`). ROW-PARALLEL and BIT-IDENTICAL to the decomposed
/// `Log(q) → Mul(p,·) → ReduceSum(axis=1) → Neg` graph (index-order sum of `p[i]·ln(q[i])`, negated, via
/// [`cross_entropy_row`]); `Log` is the same `.ln()` the `Log` primitive dispatches (scalar-bit-identical,
/// as proven by the shipped `entropy_2d`/`kl_divergence_2d`). Rows are independent so only the outer loop
/// is threaded, above the `softmax_2d_thread_count` work gate. Used by the interpreter cross-entropy
/// superinstruction. NOTE: DISTINCT from `entropy_2d` (`-Σp·log(p)`, ONE input — same 4-eq shape but the
/// `Log` reads `p`, not a second input) and from the logits-based `softmax_cross_entropy_2d` (which stages
/// a full log-softmax first). Matches the NAIVE graph, so `q=0` propagates `p·(-inf)` exactly as the graph.
#[must_use]
pub fn cross_entropy_2d(p: &[f64], q: &[f64], rows: usize, cols: usize) -> Vec<f64> {
    let _ = checked_2d_row_major_len("2D cross_entropy p", p, rows, cols);
    let _ = checked_2d_row_major_len("2D cross_entropy q", q, rows, cols);
    let mut result = vec![0.0; rows];
    if cols == 0 || rows == 0 {
        return result;
    }
    ...
    if threads <= 1 {
        for (i, slot) in result.iter_mut().enumerate() {
            let start = i * cols;
            *slot = cross_entropy_row(&p[start..start + cols], &q[start..start + cols]);
        }
        return result;
    }
...
/// Manhattan (L1) distance along the last axis of two 2D arrays, one scalar per row.
#[must_use]
pub fn manhattan_distance_2d(a: &[f64], b: &[f64], rows: usize, cols: usize) -> Vec<f64> {
    let _ = checked_2d_row_major_len("2D manhattan_distance a", a, rows, cols);
    let _ = checked_2d_row_major_len("2D manhattan_distance b", b, rows, cols);
    let mut result = vec![0.0; rows];
    if cols == 0 || rows == 0 {
        return result;
    }
    ...
    if threads <= 1 {
        for (i, slot) in result.iter_mut().enumerate() {
            let start = i * cols;
            *slot = manhattan_distance_row(&a[start..start + cols], &b[start..start + cols]);
        }
        return result;
    }
    ...
    std::thread::scope(|scope| {
        ...
        while row0 < rows {
            ...
            scope.spawn(move || {
                for (k, out_slot) in out_block.iter_mut().enumerate() {
                    let start = k * cols;
                    *out_slot = manhattan_distance_row(
                        &a_block[start..start + cols],
                        &b_block[start..start + cols],
                    );
                }
