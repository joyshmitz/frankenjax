...
impl LiteralBuffer {
    ...
    pub fn from_concat_slices(slices: Vec<(Self, usize, usize)>) -> Option<Self> {
        ...
        Some(Self {
            storage: LiteralBufferStorage::Concat {
                ...
                len,
                literals: Arc::new(OnceLock::new()),
                f64_values: Arc::new(OnceLock::new()),
    ...
    pub fn as_f64_slice(&self) -> Option<&[f64]> {
        match &self.storage {
            ...
            LiteralBufferStorage::Concat {
                ...
                .get_or_init(|| materialize_concat_f64_slices(parts, *len).map(Arc::new))
                .as_deref()
                .map(Vec::as_slice),
        }
    }
    ...
    pub fn try_into_f64_values(self) -> Result<Vec<f64>, Self> {
        match self.storage {
            LiteralBufferStorage::F64 { values, literals } => match Arc::try_unwrap(values) {
                Ok(values) => Ok(values),
                Err(values) => Err(Self {
                    storage: LiteralBufferStorage::F64 { values, literals },
                }),
            },
    ...
    pub fn as_half_float_slice(&self) -> Option<&[u16]> {
        match &self.storage {
            ...
            LiteralBufferStorage::Concat {
                ...
                .get_or_init(|| materialize_concat_half_slices(parts, *len).map(Arc::new))
                .as_deref()
                .map(Vec::as_slice),
            _ => None,
        }
    ...
    pub fn as_f32_slice(&self) -> Option<&[f32]> {
        match &self.storage {
            ...
            LiteralBufferStorage::Concat {
                ...
                .get_or_init(|| materialize_concat_f32_slices(parts, *len).map(Arc::new))
                .as_deref()
                .map(Vec::as_slice),
            _ => None,
        }
    ...
    pub fn as_i64_slice(&self) -> Option<&[i64]> {
        match &self.storage {
            ...
            LiteralBufferStorage::Concat {
                ...
                .get_or_init(|| materialize_concat_i64_slices(parts, *len).map(Arc::new))
                .as_deref()
                .map(Vec::as_slice),
        ...
        match &self.storage {
            ...
            LiteralBufferStorage::Concat {
                ...
                storage: LiteralBufferStorage::Concat {
                    ...
                    len: *len,
                    literals: Arc::clone(literals),
                    f64_values: Arc::clone(f64_values),
...
impl Serialize for LiteralBuffer {
        ...
        S: Serializer,
    {
        ...
        serialize_literal_buffer_range(&mut seq, self, 0, self.len())?;
        seq.end()
    }
}
    ...
    pub fn stack_axis0(slices: &[Value]) -> Result<Self, ValueError> {
        ...
        match &slices[0] {
            Value::Scalar(first) => {
                ...
                if let Some(tensor) = stack_axis0_dense_scalars(slices, axis_dim)? {
                    return Ok(tensor);
                }
...
fn collect_homogeneous_scalar_values<T>(
    slices: &[Value],
    mut convert: impl FnMut(Literal) -> Option<T>,
) -> Result<Option<Vec<T>>, ValueError> {
    let mut values = Vec::with_capacity(slices.len());
    for value in slices {
        let Value::Scalar(literal) = value else {
            return Err(ValueError::MixedAxisStackKinds);
        };
        let Some(converted) = convert(*literal) else {
            return Ok(None);
        };
...
fn stack_axis0_dense_scalars(
    slices: &[Value],
    axis_dim: u32,
) -> Result<Option<TensorValue>, ValueError> {
    let Some(Value::Scalar(first)) = slices.first() else {
        return Ok(None);
    };

    macro_rules! try_dense_stack {
        ($convert:expr, $build:expr) => {{
            if let Some(values) = collect_homogeneous_scalar_values(slices, $convert)? {
                return Ok(Some($build(Shape::vector(axis_dim), values)?));
            }
        }};
    }
    ...
    match *first {
        ...
        Literal::BF16Bits(_) => {
            try_dense_stack!(
                |literal| match literal {
                    Literal::BF16Bits(bits) => Some(bits),
                    _ => None,
                },
                |shape, values| TensorValue::new_half_float_values(DType::BF16, shape, values)
            );
        }
        Literal::F16Bits(_) => {
            try_dense_stack!(
                |literal| match literal {
                    Literal::F16Bits(bits) => Some(bits),
                    _ => None,
                },
                |shape, values| TensorValue::new_half_float_values(DType::F16, shape, values)
            );
        }
        Literal::F32Bits(_) => {
            try_dense_stack!(
        ...
        Literal::Complex64Bits(_, _) => {
            try_dense_stack!(
                ...
                |shape, values| TensorValue::new_complex_values(DType::Complex64, shape, values)
            );
        }
        Literal::Complex128Bits(_, _) => {
            try_dense_stack!(
                ...
                |shape, values| TensorValue::new_complex_values(DType::Complex128, shape, values)
            );
        }
    }
...
mod tests {
    ...
    #[test]
    fn stack_axis0_homogeneous_scalars_use_dense_storage() {
        let i32_stack = TensorValue::stack_axis0(&[Value::scalar_i32(-3), Value::scalar_i32(9)])
            .expect("i32 scalar stack");
        assert_eq!(i32_stack.dtype, DType::I32);
        assert_eq!(i32_stack.elements.as_i64_slice(), Some(&[-3, 9][..]));
        assert_eq!(
            i32_stack.elements.as_slice(),
            &[Literal::I64(-3), Literal::I64(9)]
        );

        let i64_stack =
            TensorValue::stack_axis0(&[Value::scalar_i64(i64::MIN), Value::scalar_i64(7)])
        ...
        let bf16_stack = TensorValue::stack_axis0(&[
            ...
            Value::Scalar(Literal::BF16Bits(0xbf80)),
        ])
        .expect("bf16 scalar stack");
        assert_eq!(bf16_stack.dtype, DType::BF16);
        assert_eq!(
            bf16_stack.elements.as_half_float_slice(),
            Some(&[0x3f80, 0xbf80][..])
        );

        let f16_stack = TensorValue::stack_axis0(&[
            Value::Scalar(Literal::F16Bits(0x3c00)),
            Value::Scalar(Literal::F16Bits(0xbc00)),
        ])
        .expect("f16 scalar stack");
        assert_eq!(f16_stack.dtype, DType::F16);
        assert_eq!(
            f16_stack.elements.as_half_float_slice(),
            Some(&[0x3c00, 0xbc00][..])
        );

        let f32_stack = TensorValue::stack_axis0(&[
            Value::Scalar(Literal::from_f32(-0.0)),
            Value::Scalar(Literal::from_f32(f32::from_bits(0x7fc0_0001))),
        ])
        .expect("f32 scalar stack");
        assert_eq!(
            f32_stack
                .elements
                .as_f32_slice()
                .expect("dense f32 scalar stack")
                .iter()
                .map(|value| value.to_bits())
                .collect::<Vec<_>>(),
            vec![(-0.0_f32).to_bits(), 0x7fc0_0001]
        ...
        let f64_stack = TensorValue::stack_axis0(&[
            Value::Scalar(Literal::from_f64(-0.0)),
            Value::Scalar(Literal::from_f64(f64::from_bits(0x7ff8_0000_0000_0042))),
        ])
        .expect("f64 scalar stack");
        assert_eq!(
            f64_stack
                .elements
                .as_f64_slice()
                .expect("dense f64 scalar stack")
                .iter()
                .map(|value| value.to_bits())
                .collect::<Vec<_>>(),
            vec![(-0.0_f64).to_bits(), 0x7ff8_0000_0000_0042]
        );

        let complex64_stack = TensorValue::stack_axis0(&[
            Value::scalar_complex64(1.5, -0.0),
            Value::scalar_complex64(-2.0, 3.25),
        ])
        .expect("complex64 scalar stack");
        assert_eq!(complex64_stack.dtype, DType::Complex64);
        assert_eq!(
            complex64_stack.elements.as_complex_slice(),
            Some(&[(1.5, -0.0), (-2.0, 3.25)][..])
        );

        let complex128_stack = TensorValue::stack_axis0(&[
            Value::scalar_complex128(1.5, -0.0),
            Value::scalar_complex128(-2.0, 3.25),
        ])
        .expect("complex128 scalar stack");
        assert_eq!(complex128_stack.dtype, DType::Complex128);
        assert_eq!(
            complex128_stack.elements.as_complex_slice(),
            Some(&[(1.5, -0.0), (-2.0, 3.25)][..])
        );

        let mixed = TensorValue::stack_axis0(&[Value::scalar_i32(1), Value::scalar_i64(2)])
    ...
    #[test]
    fn concat_dense_typed_slices_materialize_in_slice_order() -> Result<(), String> {
        let left_f64 =
            LiteralBuffer::from_f64_values(vec![1.0, -0.0, f64::from_bits(0x7ff8_0000_0000_0042)]);
        let right_f64 = LiteralBuffer::from_f64_values(vec![2.5, 3.5]);
        let mut f64_concat = LiteralBuffer::from_concat_slices(vec![
            (left_f64.clone(), 1, 2),
            (right_f64.clone(), 0, 2),
        ])
        .ok_or_else(|| "valid f64 concat slices should construct".to_owned())?;
        let f64_expected = [-0.0_f64, f64::from_bits(0x7ff8_0000_0000_0042), 2.5, 3.5];
        assert_eq!(
            f64_concat
                .as_f64_slice()
                .expect("dense f64 concat")
                .iter()
                .map(|value| value.to_bits())
                .collect::<Vec<_>>(),
            f64_expected
        ...
        let original = f64_concat.clone();
        f64_concat[0] = Literal::from_f64(9.0);
        assert_eq!(
            original.as_f64_slice().expect("clone keeps dense concat")[0].to_bits(),
            (-0.0_f64).to_bits()
        );
        assert!(
            f64_concat.as_f64_slice().is_none(),
            "mutating a lazy concat materializes literal storage"
        );

        let f32_concat = LiteralBuffer::from_concat_slices(vec![
            (LiteralBuffer::from_f32_values(vec![1.0, -0.0]), 0, 2),
            (LiteralBuffer::from_f32_values(vec![3.25]), 0, 1),
        ])
        .ok_or_else(|| "valid f32 concat slices should construct".to_owned())?;
        assert_eq!(
            f32_concat
                .as_f32_slice()
                .expect("dense f32 concat")
                .iter()
                .map(|value| value.to_bits())
                .collect::<Vec<_>>(),
            vec![1.0_f32.to_bits(), (-0.0_f32).to_bits(), 3.25_f32.to_bits()]
        );

        let i64_concat = LiteralBuffer::from_concat_slices(vec![
            (LiteralBuffer::from_i64_values(vec![1, 2, 3]), 1, 2),
            (LiteralBuffer::from_i64_values(vec![-4]), 0, 1),
        ])
        .ok_or_else(|| "valid i64 concat slices should construct".to_owned())?;
        assert_eq!(
            i64_concat.as_i64_slice().expect("dense i64 concat"),
            &[2, 3, -4]
        );

        let half_concat = LiteralBuffer::from_concat_slices(vec![
            (
                LiteralBuffer::from_half_float_values(vec![0x3c00, 0xbc00], DType::F16),
                0,
                2,
            ),
            (
                LiteralBuffer::from_half_float_values(vec![0x7e00], DType::F16),
                0,
                1,
            ),
        ])
        .ok_or_else(|| "valid half concat slices should construct".to_owned())?;
        assert_eq!(
            half_concat
                .as_half_float_slice()
        ...
        let digest = fj_test_utils::fixture_id_from_json(&(
            ...
            vec![1.0_f32.to_bits(), (-0.0_f32).to_bits(), 3.25_f32.to_bits()],
            vec![2_i64, 3, -4],
            vec![0x3c00_u16, 0xbc00, 0x7e00],
        ))
        .expect("concat dense typed-slice golden digest should build");
        assert_eq!(
            digest,
            "2865e0a49dc4739b8bc41f080918ed9114f39d6b453bef575e86becc9772a3ce"
        );

        let mixed = LiteralBuffer::from_concat_slices(vec![
            (left_f64, 0, 1),
            (LiteralBuffer::new(vec![Literal::from_f64(4.0)]), 0, 1),
        ])
        .ok_or_else(|| "valid mixed concat slices should construct".to_owned())?;
        assert!(
            mixed.as_f64_slice().is_none(),
    ...
    fn literal_buffer_streamed_serialization_matches_materialized_json() {
        ...
        assert_streamed_json_matches_materialized(LiteralBuffer::from_f64_values(vec![
            1.0,
            -0.0,
            f64::from_bits(0x7ff8_0000_0000_0042),
        ]));
        assert_streamed_json_matches_materialized(LiteralBuffer::from_f64_one_plus_x_plus_x(
            std::sync::Arc::new(vec![0.25, -0.5]),
        ));
        assert_streamed_json_matches_materialized(LiteralBuffer::from_f32_values(vec![
            1.0,
            -0.0,
            f32::from_bits(0x7fc0_0042),
        ]));
        assert_streamed_json_matches_materialized(LiteralBuffer::from_i64_values(vec![
            i64::MIN,
            -1,
            0,
            7,
        ]));
        assert_streamed_json_matches_materialized(LiteralBuffer::from_u32_values(vec![
            0,
            17,
            u32::MAX,
        ]));
        assert_streamed_json_matches_materialized(LiteralBuffer::from_u64_values(vec![
            0,
            17,
            u64::MAX,
        ]));
        assert_streamed_json_matches_materialized(LiteralBuffer::from_bool_values(vec![
            true, false, true,
        ]));
        assert_streamed_json_matches_materialized(
            LiteralBuffer::from_bool_words(vec![0b1011], 4).expect("valid bool words"),
        );
        assert_streamed_json_matches_materialized(LiteralBuffer::from_half_float_values(
            vec![0x3c00, 0xbc00, 0x7e00],
            DType::F16,
        ));
        assert_streamed_json_matches_materialized(LiteralBuffer::from_half_float_values(
        ...
        assert_streamed_json_matches_materialized(LiteralBuffer::from_complex_values(
            vec![(1.0, -2.0), (3.5, 4.25)],
            DType::Complex64,
        ));
        assert_streamed_json_matches_materialized(LiteralBuffer::from_complex_values(
