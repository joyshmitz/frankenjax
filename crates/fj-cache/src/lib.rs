...
mod tests {
    ...
    #[test]
    fn cache_manager_file_backed_evict_failure_counter_surfaces_backend_failure() {
        use super::CacheManager;

        let parent_file =
            std::env::temp_dir().join(format!("fj-cache-evict-file-{}", std::process::id()));
        std::fs::write(&parent_file, b"not a directory").expect("parent marker should write");
        let missing_dir = parent_file.join("cache");
        let key = build_cache_key(&baseline_input()).unwrap();
        ...
        assert!(
            !manager.evict(&key),
            "failed evict still reports that no entry was removed"
        );
        assert_eq!(
            manager.evict_failure_count(),
            1,
            "CacheManager must expose file-backed evict failures"
        );
        assert_eq!(manager.put_failure_count(), 0);
        assert_eq!(manager.clear_failure_count(), 0);
    ...
    fn cache_manager_lru_file_backed_evict_failure_counter_surfaces_backend_failure() {
        ...
        assert!(
            !manager.evict(&key),
            "failed LRU evict still reports that no entry was removed"
        );
        assert_eq!(
            manager.evict_failure_count(),
            1,
            "CacheManager must expose evict failures through LruCache"
        );
        assert_eq!(manager.put_failure_count(), 0);
        assert_eq!(manager.clear_failure_count(), 0);
    ...
    fn cache_manager_ttl_file_backed_put_failure_counter_surfaces_backend_failure() {
        ...
        let parent_file =
            std::env::temp_dir().join(format!("fj-cache-ttl-manager-file-{}", std::process::id()));
        std::fs::write(&parent_file, b"not a directory").expect("parent marker should write");
        let missing_dir = parent_file.join("cache");
        let key = build_cache_key(&baseline_input()).unwrap();
        ...
        assert_eq!(
            manager.get(&key),
            CacheLookup::Miss,
            "failed TTL file-cache writes must not become readable hits"
        );
        assert_eq!(
            manager.put_failure_count(),
            1,
            "CacheManager must expose put failures through TtlLruCache"
        );
        assert_eq!(manager.evict_failure_count(), 0);
        assert_eq!(manager.clear_failure_count(), 0);
    ...
    fn cache_manager_ttl_file_backed_clear_failure_counter_surfaces_backend_failure() {
        ...
        assert_eq!(
            manager.clear_failure_count(),
            1,
            "CacheManager must expose clear failures through TtlLruCache"
        );
        assert_eq!(manager.put_failure_count(), 0);
        assert_eq!(manager.evict_failure_count(), 0);
