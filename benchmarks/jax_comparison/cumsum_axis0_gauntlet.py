...
def bench(fn, args, runs=25, warm=6, inner=20):
...
# f64 [16384,1024] cumsum over axis 0 (leading/strided): 1024 independent columns, each a 16384-deep scan.
x = jnp.asarray(((np.arange(R * C) % 997 - 498) * 1e-3).astype(np.float64).reshape(R, C))
print("cumsum_axis0_f64", json.dumps(bench(lambda a: jnp.cumsum(a, axis=0), (x,)), indent=2))
