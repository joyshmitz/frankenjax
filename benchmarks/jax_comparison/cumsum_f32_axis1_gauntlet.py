...
# f32 [16384,1024] cumsum over axis 1 (innermost, contiguous, 16384 independent lines)
x = jnp.asarray(((np.arange(R * C) % 997 - 498) * 1e-3).astype(np.float32).reshape(R, C))
print("cumsum_axis1_f32", json.dumps(bench(lambda a: jnp.cumsum(a, axis=1), (x,)), indent=2))
print("cummax_axis1_f32", json.dumps(bench(lambda a: jax.lax.cummax(a, axis=1), (x,)), indent=2))
